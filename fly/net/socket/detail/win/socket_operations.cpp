...
{
    static constexpr char const s_bind_reuse = 1;
    static constexpr int const s_bind_reuse_size = static_cast<int>(sizeof(s_bind_reuse));
...
std::size_t send(fly::net::socket_type handle, std::string_view message, bool &would_block)
{
    ...
    while (keep_sending)
    {
        ...
        static constexpr std::size_t s_int_max = std::numeric_limits<int>::max();
        int const to_send_size = static_cast<int>(std::min(message.size(), s_int_max));

        int const status = ::send(handle, message.data(), to_send_size, 0);
    ...
    bool &would_block)
{
    ...
    while (keep_sending)
    {
        auto const size = static_cast<int>(std::min(packet_size, message.size()));
...
std::string recv(fly::net::socket_type handle, std::size_t packet_size, bool &would_block)
{
    ...
    int const size = static_cast<int>(packet_size);
    int const status = ::recv(handle, result.data(), size, 0);
    ...
    bool &would_block)
{
    ...
    int const size = static_cast<int>(packet_size);
    int const status = ::recvfrom(handle, result.data(), size, 0, base(address), &address_size);
    ...
    std::set<fly::net::socket_type> &reading_handles)
{
    ...
    long const usec = static_cast<long>(timeout.count());
    timeval tv {0, usec};

    // First argument of ::select() is ignored on Windows.
    int const status = ::select(0, &read_set, &write_set, nullptr, &tv) > 0;
